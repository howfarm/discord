...
# The Mini Dungeon's warning sign
MINI_DUNGEON = cv2.imread('assets/mini_dungeon.png', 0)
...
class Notifier:
    ...
    def _main(self):
        ...
        while True:
            if config.enabled:
                ...
                # Check for mini dungeon
                elite_frame = frame[height // 4:3 * height // 4, width // 4:3 * width // 4]
                elite = utils.multi_match(elite_frame, MINI_DUNGEON, threshold=0.9)
                if len(elite) > 0:
    ...
    def _1time(self, name, volume=0.5):
        """long notification for dangerous events."""

        self.mixer.load(get_alert_path(name))
        self.mixer.set_volume(volume)
        self.mixer.play(1)

    def _2time(self, name, volume=0.5):
        """long notification for dangerous events."""
